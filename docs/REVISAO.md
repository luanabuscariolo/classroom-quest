...
- **CSS herdado:** uma auditoria automática (26 estados da interface × 9 larguras de ecrã) removeu 84 declarações sobrescritas por outras regras do projeto e 13 `!important` sem efeito, com o estilo computado de todos os elementos idêntico antes e depois. Também saíram as regras de funcionalidades antigas que já não existem no HTML nem no JS (perfis de professor, `.hud-stat`, `.draw-orbit`, `.attention-note`). Restam seletores repetidos entre ficheiros que não são redundantes: a ordem da cascata decide o resultado.
